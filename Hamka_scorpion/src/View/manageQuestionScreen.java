...
public class manageQuestionScreen extends Application implements Initializable {
	...
	public void initialize(java.net.URL location, ResourceBundle resources) {
		...
		levelCombo.getItems().addAll("1", "2", "3");
		this.innerPanel.setVisible(false);
		this.removeBtn.setDisable(true);
		this.editBtn.setDisable(true);
		this.innerBtn.disableProperty()
				.bind(Bindings.isEmpty(quesField.textProperty()).or(Bindings.isEmpty(ans1Field.textProperty()))
						.or(Bindings.isEmpty(ans2Field.textProperty())).or(Bindings.isEmpty(ans3Field.textProperty()))
						.or(Bindings.isEmpty(ans4Field.textProperty())).or(Bindings.isNull(combo.valueProperty()))
	...
	public void innerBtnClicked(ActionEvent event) throws IOException {
		...
		ArrayList<Question> questionList = SysData.getInstance().getQuestionList();
		if (this.innerBtn.getText().equals("Add")) {
			Question ques = new Question(this.quesField.getText(),
					this.levelCombo.getSelectionModel().getSelectedItem(), answers, rightAnswer);
			if (questionList.contains(ques)) {
				Alert alert = new Alert(AlertType.WARNING);
				alert.setTitle("Question does exist");
			...
			Question ques = this.quesTable.getSelectionModel().getSelectedItem();
			SysData.getInstance().editQuestion(ques, this.quesField.getText(), answers,
					this.levelCombo.getSelectionModel().getSelectedItem(), rightAnswer);
	...
	public void darkTheme(boolean isOn) {
		if (isOn) {
			...
			middlePane.setStyle(
					"-fx-background-color :  #272626 ; -fx-background-radius:  12; -fx-border-radius:  10; -fx-border-color: #444444; -fx-border-width: 4;");
			...
			middlePane.setStyle(
					"-fx-background-color :  #630000 ; -fx-background-radius:  12; -fx-border-radius:  10; -fx-border-color: #444444; -fx-border-width: 4;");
